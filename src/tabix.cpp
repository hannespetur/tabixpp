...
Tabix::Tabix(std::string& file)
{
  ...
  {
    std::cerr << "[tabix++] was bgzip used to compress this file? " << file << std::endl;
    free(fnidx);
    exit(1);
  }
  ...
  {
    std::cerr << "[tabix++] the index file is older than the vcf file. Please use '-f' to overwrite or reindex." << std::endl;
    free(fnidx);
    exit(1);
  }
  free(fnidx);
  ...
  if ((fn = hts_open(cfilename, "r")) == 0) {
    std::cerr << "[tabix++] fail to open the data file." << std::endl;
    exit(1);
  }

  if ((tbx = tbx_index_load(cfilename)) == NULL) {
    std::cerr << "[tabix++] failed to load the index file." << std::endl;
    exit(1);
  }
...
void Tabix::getHeader(std::string& header)
{
  header.clear();
  kstring_t str = {0,0,0};
...
bool Tabix::setRegion(std::string& region)
{
  tbx_itr_destroy(iter);
...
bool Tabix::getNextLine(std::string& line)
{
  kstring_t str = {0,0,0};
  if (has_jumped) {
  ...
  { // step through all sequences in the file
